...
/**
 * Creates a Tiles URL template for fetching GFS data from Flux
 ...
 */
function createGFSTilesUrlTemplate({
    colorPalette,
    colorScaleRange,
    layer,
    tileWidth,
    ...
    const yy = y ?? '{y}';
    const zz = z ?? '{z}';
    return `https://compute.earthmover.io/v1/services/tiles/earthmover-public/gfs/main/solar/tiles/WebMercatorQuad/${zz}/${yy}/${xx}?&variables=${layer}&colorscalerange=${colorScaleRange.join(
        ',',
...
export default function Globe() {
    ...
    const [colorPalette, _setColorPalette] = useState('jet');
    const [colorScaleRange, _setColorScaleRange] = useState<[number, number]>([270, 320]);
